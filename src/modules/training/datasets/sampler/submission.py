"""Submission sampler the input to the given length."""

from dataclasses import dataclass
from typing import Final

import numpy as np
import numpy.typing as npt

from src.modules.training.datasets.sampler.sampler import Sampler
from src.utils.logger import logger

SAMPLING_RATE: Final[int] = 32000  # Hz
WINDOW_LENGTH: Final[int] = 5  # seconds
SOUND_LENGTH: Final[int] = 240  # 4 * 60 seconds


@dataclass
class SubmissionSampler(Sampler):
    """Extract 48(12*4) segments from the input sequence of 4 minutes."""

    def sample(self, array: npt.NDArray[np.float32]) -> npt.NDArray[np.float32]:
        """Extract 48 segments of 5 seconds each from the input sequence of 4 minutes.

        :param array: The input array.
        :return: The array with 48 segments of 5 seconds each.
        """
        # Raise a warning if the audio is not 4 minutes long (32000 samples per second)
        if len(array) != SAMPLING_RATE * SOUND_LENGTH:
            logger.warning(f"Audio is not 4 minutes long: {len(array) // SAMPLING_RATE} seconds..")
        # Extract 48 segments of 5 seconds each
        step = SAMPLING_RATE * WINDOW_LENGTH
        segments = [array[i : i + step] for i in range(0, len(array), step)]

        # If the length of the last segment is less than 80000 (2.5 sec) remove it, else pad with 0s to 160000
        if len(segments[-1]) < SAMPLING_RATE * (WINDOW_LENGTH // 2):
            segments = segments[:-1]
        else:
            segments[-1] = np.pad(segments[-1], (0, (SAMPLING_RATE * WINDOW_LENGTH) - len(segments[-1])), "constant")

        all_segments = np.array(segments)

        # If all segments are not 48, pad with zeros
        n_segments = SOUND_LENGTH // WINDOW_LENGTH
        if len(all_segments) < n_segments:
            all_segments = np.pad(all_segments, ((0, n_segments - len(all_segments)), (0, 0)), "constant")

        return all_segments[:n_segments]
